...
const PatientRecordModal = ({ isOpen, onClose, patientId, patientName }) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent
        className="max-w-4xl w-full h-[95vh] md:h-auto md:max-h-[90vh] p-0 gap-0 overflow-hidden flex flex-col rounded-t-3xl md:rounded-3xl"
        hideClose
      >
        {/* Header */}
        <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-900 shrink-0">
          ...
          <button onClick={() => onClose()} className="p-2 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-full transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50">
          ...
          {consentBlocked ? (
            ...
            <>
              ...
              <TabsList className="mx-4 mt-2 shrink-0">
                <TabsTrigger
                  value="history"
                  disabled={grantedPermissions?.medical_history === false}
                ...
                <TabsTrigger
                  value="documents"
                  disabled={grantedPermissions?.documents === false}
                  className="flex items-center gap-2"
                >
                ...
                <TabsTrigger
                  value="notes"
                  disabled={grantedPermissions?.visit_notes === false}
                  className="flex items-center gap-2"
                >
                  <ClipboardList className="w-4 h-4" /> Progress Notes
                </TabsTrigger>
              </TabsList>
                              ...
                              {documents.map(doc => (
                                <div key={doc.id} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-2xl p-4 flex items-center gap-4 hover:border-primary/30 transition-colors">
                                  ...
                                  <button
                                    ...
                                    className="p-2 text-primary hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50"
                                    title="View Document"
                                  >
                            ...
                            <div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl p-5 shadow-sm dark:shadow-slate-900/50 sticky top-0">
                              ...
                              <form onSubmit={handleSaveNote} className="space-y-4">
                                ...
                                <div>
                                  ...
                                  <textarea required rows={3} placeholder="Type your clinical notes here..." value={newNote.text} onChange={e => setNewNote({...newNote, text: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-primary/20 outline-none resize-none mt-1" />
                                </div>
                                <div>
                                  <input type="file" id="note-attachment" className="hidden" accept="image/jpeg, image/png, application/pdf" onChange={(e) => { const file = e.target.files[0]; if (file && file.size > 2 * 1024 * 1024) { toast.error("File is too large! Max 2MB."); e.target.value = ''; } else { setNewNote({ ...newNote, file }); } }} />
                                  <label htmlFor="note-attachment" className="flex items-center gap-2 text-xs font-bold text-primary cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-900/20 px-3 py-2 rounded-lg border border-dashed border-blue-200 dark:border-blue-900/30 w-full justify-center transition-colors">
                                    <Paperclip className="w-3 h-3" /> {newNote.file ? newNote.file.name : 'Attach Photo/Document (Max 2MB)'}
                                  </label>
                                </div>
