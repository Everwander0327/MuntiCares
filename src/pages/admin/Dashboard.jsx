      ...
      <div className="bg-white px-4 py-2 rounded-xl shadow-lg border border-slate-100 z-50 relative dark:bg-slate-800 dark:border-slate-700">
        ...
        {payload.map((p, i) => (
          <p key={i} className="text-sm font-semibold" style={{ color: p.color }}>
            {p.name}: {p.value}{p.name === 'Revenue' ? ' PHP' : ''}
          </p>
        ))}
      </div>
    );
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // Revenue estimation (completed requests * provider price)
        const { data: completedReqs } = await supabase
          .from('requests')
          .select('created_at, provider_id, provider:provider_id(price_per_service), status')
          .eq('status', 'Completed');
        ...
        for (let i = 5; i >= 0; i--) {
          const d2 = new Date(d.getFullYear(), d.getMonth() - i, 1);
          const key = `${monthNames[d2.getMonth()]} ${d2.getFullYear()}`;
          revenueMap[key] = { month: monthNames[d2.getMonth()], revenue: 0 };
        }
        ...
        (completedReqs || []).forEach(req => {
          ...
          if (revenueMap[key] !== undefined) {
            revenueMap[key].revenue += Number(req.provider?.price_per_service || 0);
          }
        });

        setRevenueChartData(Object.values(revenueMap));
