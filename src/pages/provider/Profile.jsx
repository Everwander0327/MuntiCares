    ...
    <DashboardLayout role="provider">
      <div className="max-w-5xl mx-auto space-y-6 md:space-y-8">
        ...
        {!profile.is_profile_complete && (
          <motion.div 
            className="bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200 text-yellow-800 p-4 md:p-5 rounded-2xl flex items-start gap-3"
        ...
        {profile.is_profile_complete && !profile.is_approved && (
          <motion.div 
            className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 text-blue-800 p-4 md:p-5 rounded-2xl flex items-start gap-3"
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
            ...
            >
              ...
              <div className="px-6 md:px-8 pt-4 pb-6 md:pb-8 text-center">
                ...
                <div className="flex items-center justify-center flex-wrap gap-2 mt-3">
                  <span className="inline-flex items-center px-3 py-1 bg-purple-50 text-purple-700 font-bold text-[10px] uppercase tracking-widest rounded-full border border-purple-100">
                    Healthcare Provider
                  </span>
                  {profile.is_approved ? (
                    ...
                    <span className="inline-flex items-center gap-1 px-3 py-1 bg-blue-50 text-blue-700 font-bold text-[10px] uppercase tracking-widest rounded-full border border-blue-100">
          ...
          <div className="lg:col-span-2 space-y-6 md:space-y-8">
            ...
            >
              <div className="p-6 md:p-8 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <h3 className="text-lg md:text-xl font-bold text-slate-900">Contact & Rates</h3>
                  <p className="text-slate-500 text-sm mt-0.5">Your professional info visible to patients</p>
                </div>
                <motion.button 
              ...
              <div className="p-6 md:p-8 space-y-5">
                {/* Bio */}
                <div>
                  <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-2">Bio / Title</p>
                  {isEditing ? (
                    <input 
                      type="text" 
                      value={editForm.bio}
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 outline-none focus:border-primary focus:bg-white focus:ring-2 focus:ring-primary/20 transition-all"
                    />
                  ) : (
                    <p className="text-slate-700 font-medium">{profile.bio || 'No bio provided'}</p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  {/* Phone */}
                  <div>
                    <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-2">Phone Number</p>
                    {isEditing ? (
                      <div className="relative group">
                        ...
                        <input 
                          ...
                          className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 pl-11 pr-4 outline-none focus:border-primary focus:bg-white focus:ring-2 focus:ring-primary/20 transition-all"
                        />
                      </div>
                    ) : (
                  ...
                  <div>
                    ...
                    {isEditing ? (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <p className="text-slate-700 font-medium">{profile.location || 'No location set'}</p>
                      </div>
                    )}
                  </div>
                  ...
                  {/* Rate */}
                  <div>
                    <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-2">Rate per Service (₱)</p>
                    {isEditing ? (
                      <div className="relative group">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="p-2 bg-slate-50 rounded-lg"><BookOpen className="w-4 h-4 text-slate-400" /></div>
                  ...
                  {/* Email */}
                  <div>
                    <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-2">Email (Read Only)</p>
                    <div className="flex items-center gap-3">
            ...
            >
              <div className="p-6 md:p-8 border-b border-slate-100">
                <h3 className="text-lg md:text-xl font-bold text-slate-900">Specializations</h3>
                <p className="text-slate-500 text-sm mt-0.5">Services you offer to patients</p>
              </div>
                ...
                {isEditing ? (
                  <div className="space-y-4">
                    <div>
                      ...
                      <textarea 
                        ...
                        className="w-full h-28 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:border-primary focus:bg-white focus:ring-2 focus:ring-primary/20 text-sm resize-none transition-all"
                      />
                    </div>
                    
                    <div>
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Suggested Services to Add:</p>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {profile.services.length > 0 ? (
                      ...
                      <p className="text-slate-500 text-sm">No specializations added yet. Click "Edit Profile" to add your services.</p>
                    )}
                  </div>
                )}
              </div>
